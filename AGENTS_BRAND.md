...
- Core library and CLI live in `src/filare/`; `wv_cli.py` exposes the `filare`/`filare-qty` commands (legacy `filare`/`filare-qty` remain for compatibility), with rendering and BOM logic in `wv_graphviz.py`, `wv_output.py`, and helpers under `tools/`.
...
- Also build the XSC harness suite with the project venv (`cd ../xsc-harnesses && FILARE=../Filare-codex1/venv-filare/bin/filare make`, or `WIREVIZ=` for legacy makefiles) to catch downstream breakage.
