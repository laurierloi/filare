...
- Add matching `just` shortcuts (e.g., `just codex-orchestrator-start`, `...-stop`, `...-tail`, `...-feedback`) that simply call `uv run filare-agents ...`; register them in `agents/extra_commands.yml` so Codex sessions surface the helpers in-context. Ensure no duplication with commands auto-generated by `scripts/generate_filare_agent_commands.py`—the generator should ingest the new `just` targets instead of hand-adding overlapping entries.
