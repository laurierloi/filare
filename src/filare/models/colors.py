...
class SingleColor(BaseModel):
    ...
    def __init__(self, inp=None, **data):
        ...
        if isinstance(inp, SingleColor):
            values = {"code_en": inp.code_en, "html": inp.html}
        elif inp is None:
            values = {"code_en": None, "html": None}
        elif isinstance(inp, int):
            hex_str = f"#{inp:06x}"
            values = {"code_en": hex_str, "html": hex_str}
        elif isinstance(inp, str):
            inp_upper = inp.upper()
            if inp_upper in known_colors:
                values = {"code_en": inp_upper, "html": known_colors[inp_upper].html}
            else:
                values = {"code_en": inp, "html": inp}
        else:
    ...
    def code_de(self):
        if not self.code_en:
            return None
        return convert_case(known_colors[self.code_en.upper()].code_de)

    @property
    def known(self):
        return self.code_en.upper() in known_colors.keys() if self.code_en else True
    ...
    def __bool__(self):
        return self.code_en is not None

    def __str__(self):
        if self.html is None:
            return ""
        elif self.known and "EN_" in color_output_mode.name:
            return convert_case(self.code_en)
...
class MultiColor:
    ...
    def html_padded_list(self):
        ...
        if padding_amount == 1:
            ...
            from filare.errors import ColorPaddingUnsupported

            raise ColorPaddingUnsupported(len(self))
