...
from wireviz.parser import parse_concat_merge_files
from wireviz.models.dataclasses import AUTOGENERATED_PREFIX
from wireviz.models.harness import Harness
from wireviz.models.utils import expand, get_single_key_and_value, smart_file_resolve
...
def _normalize_connection_set(
    connection_set, template_separator_char, designators_and_templates, autogenerated_designators
):
    ...
    if len(set(connectioncount)) > 1:
        pretty_sets = [f'length {len(v)}: "{k}": {v}' for s in connection_set for k, v in s.items()]
        prettyer_sets = "\n\t" + "\n\t".join(pretty_sets)
        raise Exception(
...
def build_harness_from_files(
    ...
    try:
        metadata = _build_metadata(yaml_file, yaml_data, extra_metadata, metadata_output_name)
    except TypeError:
        logging.error(
