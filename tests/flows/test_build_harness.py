...
import pytest
from wireviz.flows.build_harness import (
    _build_metadata,
    _collect_templates,
...
def test_metadata_typeerror_branch(monkeypatch, tmp_path, extra_metadata):
    ...
    monkeypatch.setattr("wireviz.flows.build_harness._build_metadata", boom)
...
def test_additional_bom_items_error_branch(monkeypatch, tmp_path, extra_metadata):
    ...
    monkeypatch.setattr("wireviz.models.harness.Harness.add_additional_bom_item", raise_type_error)
    with pytest.raises(TypeError):
        build_harness_from_files(
            inp=[harness],
            metadata_files=[meta],
...
"""
    ...
    monkeypatch.setattr("wireviz.models.harness.Harness.png", property(lambda self: DummyHarness.png))
    monkeypatch.setattr("wireviz.models.harness.Harness.svg", property(lambda self: DummyHarness.svg))
