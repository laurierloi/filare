...
from wireviz.flows.index_pages import build_pdf_bundle, build_titlepage
...
def test_build_shared_bom_invokes_render(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("wireviz.flows.shared_bom.generate_shared_bom", fake_generate_shared_bom)
    out = build_shared_bom(tmp_path, {"bom": "v"}, use_qty_multipliers=True, files=[tmp_path / "a"], multiplier_file_name="m.txt")
...
def test_build_titlepage_uses_metadata(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("wireviz.flows.index_pages.generate_titlepage", fake_generate_titlepage)
    build_titlepage([meta], {"titlepage": tmp_path / "titlepage"}, {"bom": "v"})
    assert called["for_pdf"] is False
    build_titlepage([meta], {"titlepage": tmp_path / "titlepage"}, {"bom": "v"}, for_pdf=True)
...
def test_build_pdf_bundle(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("wireviz.flows.index_pages.generate_pdf_output", fake_generate_pdf_output)
