...
def test_node_connector_simple_template_loads(tmp_path, monkeypatch):
    c = make_connector("X1", 2, style="simple")
...
def test_connector_loops_builds_edges():
    ...
    c.loops = [Loop(first=WireClass(pin=1), second=WireClass(pin=2), side=Side.LEFT)]
...
def test_gv_edge_wire_builds_ports(monkeypatch):
    class DummyHarness:
        def __init__(self):
            self.connectors = {"X1": make_connector("X1", 1), "X2": make_connector("X2", 1)}

    harness = DummyHarness()
    cable = Cable(designator="W1", wirecount=1)
    wire = WireClass(parent=cable, index=0, color=SingleColor("RD"))
    conn_from = WireClass(parent=harness.connectors["X1"], index=0)
    conn_to = WireClass(parent=harness.connectors["X2"], index=0)
    ...
    color, left1, left2, right1, right2 = gv.gv_edge_wire(harness, cable, DummyConnection())
